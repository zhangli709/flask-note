...
$(document).ready(function(){
    ...
    $(".filter-item-bar>.filter-area").on("click", "li", function(e) {
        ...
        //    点击时修改id的值，
            var new_id = $(this).attr('area-id');
            // 拿到页面里的aid
            var aid = location.search.split('&')[0].split('=')[1];
            var path = location.search.replace('aid='+aid, 'aid='+ new_id);
            location.href = path
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function(e) {
        ...
        //    排序操作，获取值，访问数据库，排序之后，返回到页面
            var sort_key = $(this).attr('sort-key');
            location.href = location.search.split('&sk=')[0]+'&sk='+sort_key
        }
    })
})
...
$(document).ready(function(){
   ...
   $.get('/house/allsearch/' + path, function(data){
       if(data.code=='200'){
           // 图片
           var house_info_html = template('search-house-id', {hlist:data.hlist});
           $('.house-list').html(house_info_html);

           // 地址
           var area_html = template('area-id', {alist:data.alist});
           $('.filter-area').append(area_html)
       }
   })
});